/**
 * Alpine.js Components for Blog Theme
...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('blogSearch', () => ({
        ...
        async search() {
            ...
            try {
                const response = await fetch(`/blog/search?q=${encodeURIComponent(this.query)}`);
    ...
    Alpine.data('blogPostCard', (post) => ({
        ...
        async toggleLike() {
            ...
            try {
                const response = await fetch(`/blog/post/${this.post.id}/like`, {
                    method: 'POST',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    }
        ...
        async toggleBookmark() {
            ...
            try {
                const response = await fetch(`/blog/post/${this.post.id}/bookmark`, {
                    method: 'POST',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    }
    ...
    Alpine.data('blogComments', (postId) => ({
        postId: postId,
        comments: [],
        newComment: {
            author: '',
        ...
        async loadComments() {
            ...
            try {
                const response = await fetch(`/blog/post/${this.postId}/comments`);
                const data = await response.json();
        ...
        async submitComment() {
            ...
            try {
                const response = await fetch(`/blog/post/${this.postId}/comment`, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-Requested-With': 'XMLHttpRequest'
    ...
    Alpine.data('blogPagination', (currentPage, totalPages) => ({
    ...
    Alpine.data('blogCategoryFilter', () => ({
        selectedCategory: '',
        categories: [],
        loading: false,
        ...
        filterByCategory(categoryId) {
            ...
            const url = categoryId ? `/blog/category/${categoryId}` : '/blog';
            window.location.href = url;
        }
    }));
    
    // Blog Tag Cloud Component
    Alpine.data('blogTagCloud', () => ({
        tags: [],
        loading: false,
